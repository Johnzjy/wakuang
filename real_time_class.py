...
class real_datas(object):
    def __init__(self, ax, code):
        ...
        self.mark, = ax.plot(
            [], [], lw=2, marker='h', markeredgecolor='r')  #设置箭头
        self.high_plot, = ax.plot(
            [], [], ls=':', color='r', lw=2, label='high')  #设置上限
        self.low_plot, = ax.plot(
            [], [], ls=':', color='r', lw=2, label='high')  #设置上限
        ...
        self.time_text = self.ax.text(
            0.01, 1.05, '', transform=self.ax.transAxes)
        print('init done')
    ...
    def run(self, data):
        ...
        self.line.set_data(self.xdata, self.ydata)
        self.high_plot.set_data(self.xdata[-turn:], float(
            self.data_int.high[0]))
