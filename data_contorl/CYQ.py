...
def CYQ(code, start, end, download=False, CYQ_rate=1, files_path='../report/'):
    ...
    outstanding = stock_bas.loc['%s' % code,
                                'outstanding'] * 10000 * 100  #计算流通股
    #print (datas)
    #print (cyq)
    ...
    for date in tqdm.tqdm(datas.index[1:]):
        ...
        try:
            ...
            tem_cyq.count_volume = tem_cyq.count_volume - (
                tem_cyq.count_volume * turnover // CYQ_rate)
        except:
            log_CYQ.war('\n计算换手率数据下载错误：%s自动计算计算' % date)
            try:

                turnover = datas.volume[
                    i] / outstanding * 100  # 手数 /流通手 100 = cyq rate
...
def Draw_CYQ(code_j, start_j, end_j):
    ...
    df.date = df.date.apply(
        lambda x: datetime.datetime.strptime(x, "%Y-%m-%d"))  #g改变日期格式 否则无法作图
    datas = df.set_index('date')
    ...
    plt.plot(
        datas.index, datas.close * 0 + PCT20, ls='--', linewidth=.4, color='r')
    plt.plot(
        datas.index, datas.close * 0 + PCT50, ls='--', linewidth=.4, color='r')
    plt.plot(
        datas.index, datas.close * 0 + PCT80, ls='--', linewidth=.4, color='r')
    ...
    plt.plot(
        x.count_volume, x.index * 0 + PCT20, ls='--', linewidth=.4, color='r')
    plt.plot(
        x.count_volume, x.index * 0 + PCT50, ls='--', linewidth=.54, color='r')
    plt.plot(
        x.count_volume, x.index * 0 + PCT80, ls='--', linewidth=.4, color='r')
    plt.ylim(datas.close.min(), datas.close.max())
    plt.grid(True)
