...
class Data_handle(object):
    ...
    def __init__(self):
        ...
        self.set_year=time.localtime()[0]
        self.set_mon=time.localtime()[1]
        self.quarter =0

        if self.set_mon in [1,2,3]:
            self.quarter = 1
        elif self.set_mon in [4,5,6]:
            self.quarter = 2
        elif self.set_mon in [7,8,9]:
            self.quarter = 3
        elif self.set_mon in [10,11,12]:
            self.quarter = 4
        if self.quarter == 1:
            self.set_year='%s'%(time.localtime()[0]-1)
            self.set_quarter= 4
        else :    
            self.set_quarter=self.quarter-1
        ...
        if result_down == True:
            ...
            for item,code_ in  tqdm.tqdm_gui(enumerate(data.code)):
                ...
                try:
                    ...
                    price_max=price.max()
                    price_min=price.min()
                    return_r=(price_e-price_f)/price_f*100
                    p_range=(price_max-price_min)/price_min*100
                    data.ix[item,['earnings']]=return_r.round(-1)
                    data.ix[item,['fluctuate']]=p_range.round(-1)
