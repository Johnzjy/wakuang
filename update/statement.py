...
class StockStatement(StockFolder):
    ...
    def download_statement(self,_code=None,_path=None):
        ...
        time.sleep(1)
        #print (">>>",end="")
        self.profit.to_csv(_path+"\\%s_profit.csv"%self.code,encoding="gbk")
        #print (">>>",end="")
        ...
        time.sleep(1)
        #print (">>>",end="")
        self.profit.to_csv(_path+"\\%s_balance.csv"%self.code,encoding="gbk")
        #print (">\n")
    ...
    def read_files(self,_code=None,mode="profit"):
        ...
        df=pd.read_csv(_path,encoding="gbk")# 返回一个pandas.core.frame.DataFrame 类型
        df.drop([1])
        return df
    #TODO: 加入
    def _calculate_ones_yoy(self,df=None,year_1="20180331",year_2="20170331"):
        """
        计算报表中两个报表之间的百分比
        参数： year_1 ：此期报表
              year_2  : 上期报表
        ...
        df["%s_yoy"%year_1]=(df["%s"%year_1].astype(np.float)-df["%s"%year_2].astype(np.float))/df["%s"%year_2].astype(np.float) #计算并返还Datafram
        return df
        ...
        for i in col_name[2:]:
            ...
            if str(second_year) in col_name:
                if "%s_yoy"%frist_year not in col_name:
                    yoy_df=self._calculate_ones_yoy(df=yoy_df,year_1="%s"%frist_year,year_2="%s"%second_year)
        if save is True :
            yoy_df=yoy_df.drop(["Unnamed: 0"],axis=1)
            yoy_df.to_csv(self.stock_dirs()+"\\%s_%s.csv"%(self.code,self.mode),encoding="gbk")
        return yoy_df
