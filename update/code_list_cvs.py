...
def update_list():
     ...
     for c_name in c_name_list:
         ...
         c_name_code.to_csv(path+'%s.csv'%(c_name),encoding='gbk',header=True)
     log_code_list.info('更新完成！！！！')
     return True
if __name__ == "__main__":
     ...
     log_code_list=logd.Logger(path+'code_list_cvs.log')
     log_code_list.info('%s更新code list data：%s%s'%('='*16,today,'='*16))
     update_list()
