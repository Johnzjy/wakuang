...
def calculateMa(data, Daycount):
    ...
    result = list(
        0 for x in
        data)  #used to calculate ma. Might be deprecated for future versions
...
def generateline(stocknumber, Type, startdate, enddate, interval):
    ...
    if interval != "qfq" and interval != "hfq":
        if interval == "1min" or interval == "5min" or interval == "15min" or interval == "30min" or interval == "60min":
            ...
            newdf = price_df.merge(
                vol_df, left_index=True, right_index=True).merge(
        ...
        if Type != "Kline":
            array = ts.get_k_data(
                stocknumber, start=startdata, end=enddata, ktype=interval)
            if array is None:
                return
            Type1 = firstletter(Type).encode("ascii")
        ...
        if Type != "Kline":  # 复权
            array = ts.get_h_data(
                stocknumber, start=startdata, end=enddata, autype=interval)
            if array is None:
                return
            Type1 = firstletter(Type).encode("ascii")
