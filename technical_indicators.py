...
def myMACD(price, fastperiod=10, slowperiod=20, signalperiod=9):
    ...
    bar = (
        dif - dea)  #有些地方的bar = (dif-dea)*2，但是talib中MACD的计算是bar = (dif-dea)*1
    return dif, dea, bar
...
def draw_macd(code, starttime, endtime):
    ...
    #三个指数都为正
    macd, signal, hist = myMACD(
        df['close'].values, fastperiod=10, slowperiod=20, signalperiod=9)
    ax1 = plt.subplot(111)
...
def ewma_day(code_list, days_=30):  # 30day EWMA走线
    ...
    for code_ in code_list[1:]:
        datas = ts.get_hist_data(
            code_, start='%s' % startday, end='%s' % today)
    ...
    every_day.date = every_day.date.apply(
        lambda x: datetime.datetime.strptime(x, "%Y-%m-%d"))
...
def ST_bands(code, startday, enday, tp=14):
    ...
    upperband, middleband, lowerband = talib.func.BBANDS(
        df.close.values,
        timeperiod=tp,
        nbdevup=2,
        nbdevdn=2,
...
def RSI(code='sh', startday='2015-01-05', enday='2016-12-21',
        timeperiod=10):  # price 加权平均指标
    #timeperiod=1
    if code == 'sh':
        timeperiod = 10
    df = get_date_ts(code, startday, enday)
    df['RSI'] = talib.func.RSI(df.close.values,
...
def ADOSC(code='sh', startday='2015-01-05', enday='2016-12-21', f=5,
          s=30):  # 量价分析资金流入流出
    #timeperiod=1
    # if code == 'sh':
    #    timeperiod=10
...
def MFI(code='sh', startday='2015-01-05', enday='2016-12-21',
        day=14):  # MFI - Money Flow Index
    df = get_date_ts(code, startday, enday)
    df['MFI'] = talib.func.MFI(
        high=df.high.values,
        low=df.low.values,
        close=df.close.values,
        volume=df.volume.values,
...
def CCI(code='sh', startday='2015-01-05', enday='2016-12-21', tp=11):
    df = get_date_ts(code, startday, enday)
    df['CCI'] = talib.func.CCI(
        df.high.values, df.low.values, df.close.values, timeperiod=tp)
    return df
...
def AROON(code='sh', startday='2015-01-05', enday='2016-12-21', tp=25):
    ...
    aroonup, aroondown = talib.func.AROON(
        df.high.values, df.low.values, timeperiod=tp)
...
def AROONOSC(code='sh', startday='2015-01-05', enday='2016-12-21', tp=15):
    ...
    df = get_date_ts(code, startday, enday)
    df['aroon'] = talib.func.AROONOSC(df.high.values, df.low.values, tp)
